- Fixed the historical backtest date generation logic in `backend/simulation.py::run_recommender_backtest()` by replacing calendar-day advancement with trading-day advancement using `next_trading_day()`, preventing weekend/holiday skipping gaps and resolving Monday lock-in bias. Updated `tests/backend/test_simulation.py` to assert correct test date counts.
- Fixed the recommender fingerprint mismatch pipeline bug by separating base strategy signals from filter adjustments (FII/DII flow, event risks, and sector concentration) in `backend/recommender.py`. Evaluated honest assessments on base signals only, and removed the final signals/filter_adjustments merge block in `recommend()`. This ensures database insertion of paper/shadow trades and background cron jobs use the correct clean base strategy fingerprint (`FP_base`), resolving cache misses and data-tier degradation. Updated unit tests in `tests/backend/test_recommender.py` to verify correct separate lists of base signals and filter adjustments.
- Fixed signal direction mismatch for filter adjustments in `backend/recommender.py::_recompute_confidence_and_counts()` by excluding post-processing filter adjustments (market bias, event risk, sector concentration) from the technical signal counts and confidence calculation by default, preventing artificial inflation of setup confidence tags and ensuring high-quality calibration data for shadow trades.
- Fixed double-counting of trades in `backend/cron.py::recompute_fingerprints_and_features_for_last_180_days()` by introducing deterministic Python deduplication of duplicate paper and shadow trades on `(ticker, entry_date, signal_fingerprint)` (prioritizing paper trades) during the cache rebuild, preserving distinct signal setups for the same stock on the same day.
...
- Fixed the signal model training data query in `backend/signal_model.py` to retrieve `signal_fingerprint`, and implemented deterministic Python deduplication prioritizing paper trades over shadow trades using the `(ticker, entry_date, fingerprint)` key (computing fingerprints dynamically for null fallbacks) to preserve distinct setups for the same stock on the same day during model training.
...
- Fixed double-counting of trades in `backend/honest_assessment.py::get_honest_assessment()` by introducing deterministic Python deduplication for both direct database fallback queries and Kelly stats estimation on the `(ticker, entry_date, fingerprint)` key, prioritizing paper trades over shadow trades.
